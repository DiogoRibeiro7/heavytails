...
class Zipf(Samplable):
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "_Z", sum(n ** (-self.s) for n in range(1, self.kmax + 1)))
...
class YuleSimon(Samplable):
    ...
    def pmf(self, k: int) -> float:
        if k < 1:
            return 0.0
        return self.rho * math.gamma(k) * math.gamma(self.rho + 1) / math.gamma(k + self.rho + 1)
...
class DiscretePareto(Samplable):
    ...
    def __post_init__(self) -> None:
        ...
        H = sum((k / self.k_min) ** (-self.alpha) for k in range(self.k_min, self.k_max + 1))
    ...
    def cdf(self, k: int) -> float:
        ...
        return sum(((n / self.k_min) ** (-self.alpha)) for n in range(self.k_min, k + 1)) / self._H
